...
validate <- function(path, simple = TRUE) {
  ...
  for (i in 1:length(RMassBank.env$mb)) {
    if (RMassBank.env$mb[[i]]@compiled_ok[[1]][["AC$MASS_SPECTROMETRY"]][["MS_TYPE"]] == "MS2" || RMassBank.env$mb[[i]]@compiled_ok[[1]][["AC$MASS_SPECTROMETRY"]][["MS_TYPE"]] == "MS") {
      tests[[i]] <- RUnit::defineTestSuite(Files[i],
        dirs = system.file(package = "RMassBank", "validationTests"), testFileRegexp = "runit.MS2.test.R",
        # testFuncRegexp = "^test.+",
      ...
      tests[[i]] <- RUnit::defineTestSuite(Files[i],
        dirs = system.file(package = "RMassBank", "validationTests"), testFileRegexp = "^runit.MSn.test.[rR]$",
        # testFuncRegexp = "^test.+",
        rngKind = "Marsaglia-Multicarry",
        rngNormalKind = "Kinderman-Ramage"
      )
  ...
  RUnit::printHTMLProtocol(testData, fileName = file.path(getwd(), "report.html"))
  if (simple) {
    fileConnection <- file(file.path(getwd(), "report.html"), open = "r")
    ...
    fileConnection <- file(file.path(getwd(), "report.html"), open = "w")
...
.unitTestmzR <- function(WD = getwd()) {
  ...
  w <- msmsWorkflow(w,
    mode = "pH", steps = c(1:4), archivename =
      "pH_narcotics"
  )
  ...
  w <- msmsWorkflow(w,
    mode = "pH", steps = 4, archivename =
      "pH_narcotics", newRecalibration = FALSE
  )
  w <- msmsWorkflow(w,
    mode = "pH", steps = c(5:8), archivename =
      "pH_narcotics"
  )
  mb <- newMbWorkspace(w)
  ...
  testSuite <- RUnit::defineTestSuite("Electronic noise and formula calculation Test",
    dirs = system.file("unitTests",
      package = "RMassBank"
    ), testFileRegexp = "runit.EN_FC.R",
    # testFuncRegexp = "^test.+",
    rngKind = "Marsaglia-Multicarry",
    rngNormalKind = "Kinderman-Ramage"
  )
  testSuite2 <- RUnit::defineTestSuite("Evaluation of data acquisition process",
    dirs = system.file("unitTests",
      package = "RMassBank"
    ), testFileRegexp = "runit.DA.R",
    # testFuncRegexp = "^test.+",
    rngKind = "Marsaglia-Multicarry",
    rngNormalKind = "Kinderman-Ramage"
  )
...
.unitTestRMB2 <- function(WD = getwd()) {
  ...
  w <- msmsWorkflow(w,
    mode = "pH", steps = c(1), archivename =
      "pH_narcotics"
  )
  w <- newStep2WorkFlow(w, mode = "pH", analyzeMethod = "formula")
  w <- msmsWorkflow(w,
    mode = "pH", steps = c(3:4), archivename =
      "pH_narcotics"
  )
  storedW <- loadMsmsWorkspace(system.file("results/pH_narcotics_RF.RData",
    package = "RMassBankData"
